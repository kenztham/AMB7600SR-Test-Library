		...
		try
		{
			...
			for each(TestProgramData^ tpdata in tpdatas)
			{
				...
				if (tpdata->Exception != nullptr)
				{
					...
					if (glob->AWV.Debug) WriteToTcrLgr("SITE " + tpdata->siteIndex.ToString(), tpdata->Exception->ToString());
					throw gcnew Aemulus::Hardware::AlarmException(tpdata->Exception->ToString(), tpdata->ErrorCode);
				}
			}
		}
		finally
			...
			* Create dedicated thread based on the number of test site oer DC, since functional test will thread/ run in paralle with DC */
			...
			for each(TestProgramData^ tD in tDs)
			{
				...
				if (tD->Exception != nullptr)
				{
					...
					if (glob->AWV.Debug) WriteToTcrLgr("SITE " + tD->siteIndex.ToString(), tD->Exception->ToString());
					throw gcnew Aemulus::Hardware::AlarmException(tD->Exception->ToString(), tD->ErrorCode);
				}
			}
		}
		finally
